...
export async function GET(request: NextRequest) {
  try {
    ...
    const tokens = await exchangeCodeForTokens('salesforce', code);
    ...
    const response = NextResponse.redirect(
      new URL(
        '/dashboard/settings/integrations?success=Salesforce+connected+successfully',
        request.url
      )
    );
    ...
    return NextResponse.redirect(
      new URL(
        `/dashboard/settings/integrations?error=${encodeURIComponent('Failed to connect Salesforce')}`,
        request.url
      )
    );
  }
